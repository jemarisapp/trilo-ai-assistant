...
def setup_database():
    ...
    print("Generating CFB matchups...")
    all_matchups = set(f"{t1} vs {t2}" for t1, t2 in permutations(teams, 2))

    print("Inserting CFB matchups into the database...")
    cursor.executemany("INSERT OR IGNORE INTO \"cfb-matchups\" (matchup) VALUES (?)", [(m,) for m in all_matchups])

    conn.commit()
    conn.close()
    print(f"CFB matchups setup complete. {len(all_matchups)} matchups created.")
